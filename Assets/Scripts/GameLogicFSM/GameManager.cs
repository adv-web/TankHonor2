...
public class GameManager : PunBehaviour
{
    ...
    public void InstantiatePlayer()
    {
        ...
        localPlayer.GetComponent<TankMove>().enabled = true;					//启用PlayerMove脚本，使玩家对象可以被本地客户端操控
        TankShoot tankShoot = localPlayer.GetComponent<TankShoot>();		//获取玩家对象的PlayerShoot脚本
        tankHealth = localPlayer.GetComponent<TankHealth>();				//获取玩家对象的PlayerHealth脚本

        Transform tempTransform = localPlayer.transform;
        mainCamera.transform.parent = tempTransform;							//将场景中的摄像机设为玩家对象的子对象
        mainCamera.transform.localPosition = new Vector3(0,2.73f,-0.31f);		//设置摄像机的位置，为PlayerShoot脚本中的射击起始位置
    ...
    {
        TankHealth ph = localPlayer.GetComponent<TankHealth>();
