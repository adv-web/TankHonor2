...
public class TankShoot : PunBehaviour
{
    ...
    public int shootingDamage = 100;             //射击伤害
    public float shootingRange = 500.0f;         //射击范围
    public float timeBetweenShooting = 2.0f;	//射击间隔
    ...
    void Update()
    {
        if (!photonView.isMine || !tankHealth.isAlive)    //如果玩家对象不属于本地客户端，或者玩家已死亡，解说函数执行
            return;
        ...
        {   //玩家按下射击键，与上次射击的间隔超过射击间隔
            ...
            photonView.RPC("Shoot", PhotonTargets.MasterClient, PhotonNetwork.player, Barrel_MainPart.position, gunBarrelEnd.position);	//使用RPC,调用MasterClient的Shoot函数，函数参数为发起射击的玩家
        ...
        if (Physics.Raycast(ray, out hitInfo, shootingRange))
        {
            ...
            if (go.tag == "Player")//如果击中玩家
            {
                ...
                if (tankHealth.team != GetComponent<TankHealth>().team)
                {
                    ...
                    GameManager.gm.UIController.updateEnemyHpUI(tankHealth.currentHP, tankHealth.maxHP);
                }
    ...
    void Shoot(PhotonPlayer attacker, Vector3 origin, Vector3 gunEnd)
    {
        if (!PhotonNetwork.isMasterClient)      //如果玩家不是MasterClient，结束函数的执行
            return;
        //Vector3 rotatedPos = transform.rotation * shootingPosition; //随着角色的旋转，射击的初始位置也需要跟着旋转
        ray.origin = origin;	//设置射击射线的起始端点
        ray.direction = gunEnd - origin;				//设置射击射线的方向
        ...
        {   //如果射线击中游戏对象
            ...
            {								//如果击中玩家
                TankHealth tankHealth = go.GetComponent<TankHealth>();
                tankHealth.TakeDamage(shootingDamage, attacker);
                /*if (tankHealth.team != GetComponent<TankHealth>().team)
                {   //如果被击中玩家队伍与攻击者玩家队伍不同
                    // playerHealth.TakeDamage(shootingDamage, attacker);          //被击中玩家扣血
                }*/
            ...
            bulletEffectPosition = ray.origin + shootingRange * ray.direction;  //如果未击中，子弹爆炸效果为子弹失效的位置
        Debug.DrawLine(ray.origin, bulletEffectPosition, Color.yellow);
        fire(ray.origin, bulletEffectPosition);
        if (lineObj != null)
        {
            lineObj.SetWidth(0.02F, 0.02F);
            lineObj.SetPosition(0, ray.origin);
    ...
    void fire(Vector3 origin,Vector3 hitposition)
    {
        //GameObject bulletObject = Instantiate(bulletEffect, origin, mainGunTransform.rotation) as GameObject;
        GameObject bulletObject = Instantiate(bulletEffect, origin, mainGunTransform.rotation) as GameObject;

        Vector3 tempVelocity = hitposition-origin;
        bulletObject.GetComponent<Rigidbody>().velocity = tempVelocity/tempVelocity.magnitude*200.0f;
        bulletObject.GetComponent<BulletController>().lifeTime = tempVelocity.magnitude/200.0f;
        Debug.Log("炮弹速度" + bulletObject.GetComponent<Rigidbody>().velocity);
        Debug.Log("炮弹生命周期" + bulletObject.GetComponent<BulletController>().lifeTime);
    }
    //RPC函数，显示射击效果
    /*[PunRPC]
