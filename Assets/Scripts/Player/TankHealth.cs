...
public class TankHealth : PunBehaviour
{
    ...
    void SetInvincible(bool isInvincible)
    {
        ...
        {
            if (team != GameManager.gm.localPlayer.GetComponent<TankHealth>().team)
    ...
    void UpdateHP(int newHP)
    {
        ...
        {   //如果玩家已死亡
            ...
            {                   //如果是本地客户端
                ...
                Invoke("TankSpawn", respawnTime);	//使用invoke函数，复活玩家
                GameManager.gm.UpdateDeath();
            }
            else
            {
                Invoke("TankDisable", disableTime);
    ...
    void TankSpawn()
    {
        photonView.RPC("TankReset", PhotonTargets.AllViaServer);  //使用RPC，初始化复活时的玩家属性
