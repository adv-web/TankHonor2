...
public class TalentSkillPanelController : MonoBehaviour {
    ...
    void OnEnable () {
		...
		if (PlayFabUserData.lv >= 3)
			maskImages[0].SetActive(false);
		if (PlayFabUserData.lv >= 6)
			maskImages[1].SetActive(false);
		if (PlayFabUserData.lv >= 9)
			maskImages[2].SetActive(false);
		if (PlayFabUserData.lv >= 12)
			maskImages[3].SetActive(false);
    ...
    void UpdateTalentSkill(Sprite skillSprite)
    {
		...
		case "ExpAndMoneySkill":
			lv = PlayFabUserData.expAndMoneySkillLV;
			break;
		case "ShootingRangeSkill":
			lv = PlayFabUserData.shootingRangeSkillLV;
			break;
		case "ShootingIntervalSkill":
			lv = PlayFabUserData.shootingIntervalSkillLV;
			break;
		case "ShootingDamageSkill":
			lv = PlayFabUserData.shootingDamageSkillLV;
			break;
		}
